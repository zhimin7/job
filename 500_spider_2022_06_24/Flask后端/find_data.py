...
def find_endData(mytime):
    ...
    if mytime==[]:
        ...
        yesterday_time = time.strftime("%Y-%m-%d", time.localtime(y_time)) + ' 00:00:00'
        now_time = time.strftime("%Y-%m-%d", time.localtime(t_time)) + ' 23:59:59'
        sql = f'select fid, gamestime,home_team,score,away_team from now_data_end where  gamestime<="{now_time[5:16]}" and gamestime>="{yesterday_time[5:16]}" order by gamestime;'
        cursor.execute(sql)
        res = cursor.fetchall()
        mylist=[]
        for i in res:
            mylist.append({'time':i[1],'home_team':i[2],'score':i[3],'away_team':i[4]})
        return mylist

    elif mytime!=[]:
        sql = f'select fid, gamestime,home_team,score,away_team from now_data_end where  gamestime<="{mytime[1]}" and gamestime>="{mytime[0]}"order by gamestime;'
        cursor.execute(sql)
        res = cursor.fetchall()
        mylist=[]
        for i in res:
