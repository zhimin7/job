...
def create_table_login():
    db = pymysql.connect(host=host, user=user, passwd=passwd, port=port, db=mydb)
    cursor = db.cursor()
    sql='create table if not exists login_info(username varchar(255) null ,loginTime varchar(255) null ,loginAddress varchar(255) null)'
    cursor.execute(sql)
    db.close()


# 保存登录信息数据
def saving_table_login(srr):
    create_db_login()
    db = pymysql.connect(host=host, user=user, password=passwd, port=port, db=mydb)
    cursor = db.cursor()
    sql = 'insert into login_info(username, loginTime,loginAddress, province, city, region, addr) values(%s,%s,%s,%s,%s,%s,%s)'
    try:
