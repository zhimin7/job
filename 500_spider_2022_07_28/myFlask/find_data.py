...
# port = 3306
# mydb='lottery_7_28'
from saving_data import saving_table_login
...
# 登录验证
def find_loginuser(username, password,loginAddress):
    db = pymysql.connect(host=host, user=user, passwd=passwd, port=port, db=mydb)
    cursor = db.cursor()
    sql = f'select * from usertable where  username="{username}" and password="{password}";'
    cursor.execute(sql)
    res = cursor.fetchall()
    response = requests.get(f'https://whois.pconline.com.cn/ipJson.jsp?ip=http://{loginAddress}/&json=true')
    data = response.json()
    pro = data.get('pro', '')
    city = data.get('city', '')
    ...
    if res != ():
        mylist = ['123', '200', '登录成功']
        loginTime=str(datetime.now())[:19]
        login_list=[username,loginTime,loginAddress, pro, city, region, addr]
        saving_table_login(login_list)
        return mylist
    elif res == ():
        mylist = ['', '404', '账号或密码错误']
        return mylist