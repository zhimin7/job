...
import { find_front30 } from "@/api/getdata.js";
...
const findGamesDataSpiderList = async () => {
  const res = await find_front30(queryForm.value);
...
const deriveExcel = async () => {
  ...
  const filename =
    "500彩票-比赛前30分钟数据-" + state.date + " " + state.time + "分.xlsx";
