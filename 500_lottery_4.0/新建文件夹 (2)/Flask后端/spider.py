...
def get_end_data():
    ...
    for td in tds:
        ...
        # 当比赛结束时
        if now_time_stamp-3600*3 <= games_time < now_time_stamp + 3600 * 29 and now_state_end == '完':
            fid = end_temp['fid']
            sql1 = f'select * from now_data_end  where fid="{fid}";'
            cursor.execute(sql1)
            res = cursor.fetchall()
            if res == ():
                end_temp['delayed_time']=''
                end_temp['spider_time']=''
                ...
                end_temp['handicap']=''
                end_temp['All_companies'] = ''
                end_temp['BiFa'] = ''
                end_temp['Matchbook'] = ''
                end_temp['Leon'] = ''
                end_temp['Betsson'] = ''
                end_temp['mainstream_companines_3'] = ''
                end_temp['mainstream_companines_1'] = ''
                end_temp['Exchange'] = ''
                print(end_temp)
